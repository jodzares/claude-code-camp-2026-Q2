...
These are telnet login credentials, not play-styles or personas. Log in as
one of these accounts by passing its username and password to `mud.py start`
via `--name <username> --password <password>`:
...
Default if unspecified: `dummy`/`helloworld`. Always pass `--name` explicitly
when a specific account is requested — do not rely on the default.
