...
Python port of `week1_baseline/ruby/03_prompt_builder`. Behaviour matches the
Ruby version; see `docs/plans/python_port/03_prompt_builder.md` for the full
...
`requirements.txt` installed (same environment as `02_the_registry`), plus a
`.env` in `.boukensha/` with the configured provider's API key present (it
is read, not called, this step) and `.boukensha/settings.yaml` pointing
`tasks.player.provider`/`model` at a supported combination.
